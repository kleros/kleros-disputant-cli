...
- [ ] The `Error(string)` branch bounds the decoded reason before it becomes guidance. A reason that
...
- [ ] The unmapped-selector branch bounds the raw hex it interpolates **while keeping the 4-byte
      selector intact and unbounded**. The selector is the part a reader looks up; the trailing data
      is what runs to kilobytes.
- [ ] The bound is 160 characters, the same number `ADR-0013` chose for the RPC cause, and it is
...
- [ ] Control characters are removed from the bounded fragment: C0 (newline, carriage return and the
...
- [ ] `DecodedRevert.data` keeps the revert data **verbatim and unbounded**. `spec/01 §5` requires
...
- [ ] The tension that creates is resolved explicitly, not silently: `spec/01 §5` currently discharges
...
- [ ] `error.shortMessage || error.message` narrows to `shortMessage` alone with a fixed fallback when
...
- [ ] Tests pin each bound against a payload that exceeds it: an oversized `Error(string)`, an
...
- [ ] A test pins that a mapped Kleros error and a matched `GUIDANCE_BY_REASON` row are **not**
...
- [ ] `spec/03 §5.1` rule 4 gains revert data. It reads "Output is kept small. The template body and
...
- [ ] A new ADR records the decision and, importantly, the reachability finding behind it: that no
...
- [ ] `pnpm test`, `pnpm typecheck`, `pnpm lint` and `pnpm build` stay clean, and `pnpm test:fork` is
