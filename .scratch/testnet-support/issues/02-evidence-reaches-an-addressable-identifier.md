...
**Status:** done, 2026-09-09 — `ADR-0014`, `spec/02 §4.2`, `spec/05 §1.6a`

- [x] `submit-evidence` resolves the core dispute ID to the arbitrable's local dispute ID and submits
      the local one.
- [x] The dispute's arbitrable is checked **before** the mapping is trusted. The core-to-local
...
- [x] A new refusal, `DISPUTE_NOT_ADDRESSABLE`, covers a dispute that exists but was created by a
...
- [x] No additional round trip. The dispute record is already fetched in the first multicall and
      carries the arbitrable; the mapping takes only the core dispute ID, so it joins that same call.
- [x] Tests use the measured divergence — a dispute record naming a foreign arbitrable, and a mapping
      returning 33 for core dispute 58 — and are written against the **v2 Beta** double, because the
      defect is reachable there.
- [x] At least one path reaches the new refusal through the JSON-RPC double, so the HTTP client
      builds the real request, as the verification specification requires.
- [x] A new ADR records the resolution, the measurements behind it, and that this is a Beta defect the
      testnet exposed rather than testnet scope.
- [x] The read-scope ADR is amended: reads may determine **what** to sign, not only **whether** to
      sign, where the alternative is a write that cannot be read back.
- [x] The evidence-policy ADR gains a line for the second hard refusal. Its "warns and never refuses"
      policy is unchanged in substance — the exception has always been unreachability.
- [x] The payload-construction section of the specification records which identifier is submitted and
